...
find /home -name "authorized_keys" > "$OUTPUT_DIR/ssh_auth_keys.txt"
...
find / -perm -4000 > "$OUTPUT_DIR/setuid_files.txt" 2>/dev/null

# -------- Persistence --------
ls /etc/systemd/system/* > "$OUTPUT_DIR/persistence_systemd.txt"
ls /var/spool/cron/* > "$OUTPUT_DIR/spool_cron.txt" 2>/dev/null
...
find / -type f -printf '%T+ %p\n' | sort -r | head -n 100 > "$OUTPUT_DIR/timeline_logs.txt" 2>/dev/null
strings /dev/mem > "$OUTPUT_DIR/ram_capture.txt" 2>/dev/null

find / -type f \\( -name "*.encrypted" -o -name "*.locky" -o -name "*.crypt" \\) > "$OUTPUT_DIR/ransomware_extensions.txt" 2>/dev/null
...
<style>
body{margin:0;background:#0b0c10;color:#c5c6c7;font-family:sans-serif}
nav{position:fixed;top:0;width:100%;background:#1f2833;padding:10px;white-space:nowrap;overflow-x:auto;z-index:1000;border-bottom:1px solid #45a29e}
...
echo "<footer><p>🔧 Report Generated by <strong>Forensiq</strong> | Developer: Tarun Sharma</p></footer></div></body></html>" >> "$HTML_REPORT"
